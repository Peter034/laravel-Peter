...
function filter() {
    ...
    flag = 0
    initialData.forEach(data => {
        if (categoryChoosen == 0 || data.categoryId == categoryChoosen){
        ...
        displayProduct = displayProduct +
            `
              <div class="col-md-6 col-lg-3 mb-5 productContainer">
                ...
                <button class="deleteBtn" value=${data.prodId}>Delete</button>
              </div>
            `
    })
...
function deleteItem(){
  ...
  deleteBtn.forEach(btn => {
    btn.addEventListener('click', (evt)=>{
      deleteApi(evt.target.value)
      getApiData()
    })
  })
}

function deleteApi(prodId){
